...
typedef NS_ENUM(NSUInteger, DMTabBarItemSelectionType)
{
...
// This is the event called by DMTabBar when an events occur.
// DMTabBar will post two kinds of events, when a selection will change
// and when a change has occurred
...
@property (strong) NSArray *tabBarItems;

// change selected item by passing a DMTabBarItem object
// (ignored if selectedTabBarItem is not contained inside tabBarItems)
@property (assign) DMTabBarItem *selectedTabBarItem;
